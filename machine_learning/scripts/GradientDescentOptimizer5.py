...
def function(x,aa,a,b,c,d):
	#return a*x**3+b*x**2+c*x+d
  return aa*x**4+a*x**3+b**2+c*x+d

#placeholderは後から値を入れれる関数
x = tf.placeholder(tf.float32, shape=[None,1])
y = tf.placeholder(tf.float32, shape=[None,1])
#Variableでa,bを変数で宣言
aa = tf.Variable(tf.random_normal(shape=[]))
...
#平均の計算式
E = tf.reduce_mean((function(x,aa,a,b,c,d)-y)**2,0)
...
xn = randn(10,1)
#feed_dictでx,yに与える値
feed_dict = {x:xn,y:function(xn,aan,an,bn,cn,dn)+randn(10,1)}
#200回行う，pltを使ってグラフを可視化
for i in range(200):
	print session.run(step, feed_dict)
	print session.run(E, feed_dict)
	aa_,a_,b_,c_,d_ = session.run([aa,a,b,c,d], feed_dict)
	print [aa_,a_,b_,c_,d_]
	plt.clf()
	plt.scatter(feed_dict[x],feed_dict[y])
	plt.hold('on')
	xi = np.linspace(-3,3,100)
	plt.plot(xi,np.array([function(xi,aa_,a_,b_,c_,d_),function(xi,aan,an,bn,cn,dn)]).transpose())
	plt.pause(0.001)
